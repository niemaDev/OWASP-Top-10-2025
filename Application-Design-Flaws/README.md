...
No one builds an app completely from scratch anymore. Every project pulls in many of pieces built by other people code libraries, plugins, even AI models. A supply chain failure is when one of those borrowed pieces turns out to be compromised, outdated, or was never checked properly in the first place. The most scary part? The vulnerability isn't even in *your* code  it's in something you trusted and plugged in.
...
Check components before you use them  don't just grab and go. Keep everything patched, but also verify updates are genuinely legitimate not malicious software trying to still your file or system before installing them check the digital signatures. Lock down your build systems so outsiders can't slip something in. Keep a record of where every piece of your software came from. And build supply chain risks into your security thinking from day one, not as an afterthought.

### My walkthrough

The next challenge was: *navigate to 10.112.154.11:5003. The code is outdated and imports an old `lib/vulnerable_utils.py` component. Can you debug it?*
...
Because of the encryption is quiet protecting almost everything  your passwords, your messages, your card details, your login sessions. When it goes wrong , it doesn't fail small. Attackers can stills our data mid-transit, crack weak passwords in bulk, or just find secrets sitting around in plain text.
...
**So what actually is this?**
Imagine if a bahir dar university student result (grade) portal  where you log in with just your student ID number to see your grades .no password, because "only students know their own ID."

Except student IDs aren't secret — they're printed on ID cards, written on exam papers, visible in class attendance sheets. The system was designed assuming an ID number is private information, when it never really was. That's insecure design: a system built on a wrong assumption about what counts as a secret, not a coding bug.
 The issue was never designed with security in mind in the first place. Nobody asked "how could this be abused?" before building it.

That's insecure design. It's not "something broke." It's "nobody thought about keeping the bad guys out before they built it."

**Why should you care?**

cause bugs in a code can be fixed with a quick patch. but Bad or wrong design doesn't. If the whole system's logic think users will always behave noble, or that an AI model will always give safe and correct answers, that assumption is baked in everywhere  and ripping it out means redesigning how the whole system thinks, not just tweaking a line of code.
...
Treat every AI model like it can't be trust worthy until it proves otherwise. Check both what go into a model and what comes out of it. Keep a model's instructions separate from whatever a user types in, so users can't rewrite the rules. Have a human double-check anything high-risk before it happens automatically. Think about how a systems feature could be abused *before* you build it. And give every user, API, and service only the minimum access it actually needs to do its job — nothing extra "just in case."

### My walkthrough
![secure chat mobile app](images/securechat.png)


![admin api page](images/5005-api.png)

![admin API for users](images/users.png)

![flag](images/admin-flag.png)
